...
const CorralonesContainer = () => {
  ...
  const [Corralon, setCorralon] = useState(null);
  ...
  const handleDeletUser = async () => {
    const res = await DeleteElement("Corralones", Corralon.docId);
            ...
            <DrawerBody>
              <Box display="flex" flexDir="column" gap="2">
                <Box>
                  ...
                  <Text fontWeight="light"> {Corralon.Direccion}</Text>
                </Box>
                <Box>
                  <Badge colorScheme="blue">Contacto:</Badge>
                  <Text fontWeight="light"> {Corralon.Contacto}</Text>
                  <Badge colorScheme="blue">Celular:</Badge>
                  <Text fontWeight="light"> {Corralon.Celular}</Text>
                </Box>
                <Box gap="2">
                  <Badge colorScheme="blue">Disponibilidad:</Badge>
                  <Text fontWeight="light"> {Corralon.Rol}</Text>
                  <Badge colorScheme="blue">Region:</Badge>
                  <Text fontWeight="light"> {Corralon.Region}</Text>
                </Box>

                <Box w="full">
                  <MapPoints coordinates={Corralon.Coordenadas} />
                </Box>
              </Box>
            </DrawerBody>
        ...
        <AlertDialog isOpen={isDelet} onClose={() => setIsDelet(false)}>
          <AlertDialogOverlay>
            <AlertDialogContent>
              ...
              <AlertDialogBody>
                ¿Deseas eliminar al corralon {Corralon.Nombre}?
              </AlertDialogBody>
